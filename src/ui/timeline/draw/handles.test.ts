...
describe('drawHandles', () => {
  it('dims the region left of "in" and right of "out"', () => {
    ...
    drawHandles(ctx, 1000, { inX: 100, outX: 800, heightPx: 236, inFill: color.accent, outFill: color.accent });
  ...
  it('draws no dim rect when in/out reach the canvas edges', () => {
    const { ctx, calls } = makeRecordingCtx();
    drawHandles(ctx, 1000, { inX: 0, outX: 1000, heightPx: 236, inFill: color.accent, outFill: color.accent });
    expect(calls.filter((c) => c.includes(color.dim))).toHaveLength(0);
  });

  it('draws each handle bar with its own passed-in fill color', () => {
    const { ctx, calls } = makeRecordingCtx();
    drawHandles(ctx, 1000, { inX: 100, outX: 800, heightPx: 236, inFill: color.accent, outFill: color.accentActive });
