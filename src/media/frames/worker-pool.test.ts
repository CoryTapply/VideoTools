...
describe('FrameWorkerPool initialStaggerMs', () => {
  it('defaults to every handle free immediately (no stagger)', () => {
    const handleA = new FakeWorkerHandle();
    const handleB = new FakeWorkerHandle();
    const pool = new FrameWorkerPool([handleA, handleB]);
    void pool.submit(request(1));
    void pool.submit(request(2));
    expect(handleA.decodeCalls).toHaveLength(1);
    expect(handleB.decodeCalls).toHaveLength(1);
  });

  it('delays every handle after the first from accepting work by initialStaggerMs * its index', () => {
    vi.useFakeTimers();
    try {
      const handleA = new FakeWorkerHandle();
      const handleB = new FakeWorkerHandle();
      const handleC = new FakeWorkerHandle();
      const pool = new FrameWorkerPool([handleA, handleB, handleC], { initialStaggerMs: 100 });

      void pool.submit(request(1));
      void pool.submit(request(2));
      void pool.submit(request(3));

      // Only the first handle is free at t=0 -- jobs 2 and 3 sit queued, not dispatched to a
      // hardware decode session that hasn't been given time to finish initializing yet.
      expect(handleA.decodeCalls).toHaveLength(1);
      expect(handleB.decodeCalls).toHaveLength(0);
      expect(handleC.decodeCalls).toHaveLength(0);
      ...
      vi.advanceTimersByTime(100);
      expect(handleC.decodeCalls).toHaveLength(1);
      expect(pool.queuedCount).toBe(0);
    } finally {
      vi.useRealTimers();
    }
  ...
  it('a staggered handle joining after dispose() does not resurrect a terminated pool', () => {
    vi.useFakeTimers();
    try {
      const handleA = new FakeWorkerHandle();
      const handleB = new FakeWorkerHandle();
      const pool = new FrameWorkerPool([handleA, handleB], { initialStaggerMs: 100 });
      pool.dispose();
